		...
		&schema.Schema{
			...
			Elem: &schema.Resource{
				Schema: map[string]*schema.Schema{
					string(VirtualNetworkName): &schema.Schema{
						Type:     schema.TypeString,
						Required: true,
					},

					string(SubnetName): &schema.Schema{
						Type:     schema.TypeString,
						Required: true,
					},
					...
					string(AssignPublicIP): &schema.Schema{
...
func expandAzureGroupNetwork(data interface{}) (*azure.Network, error) {
	...
	if list != nil && list[0] != nil {
		...
		if v, ok := m[string(AssignPublicIP)].(bool); ok {
			// todo this will default to false when not set, is that the desired behavior?
